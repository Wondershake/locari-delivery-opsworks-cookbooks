...
execute "delete recode from security group" do
  environment(
    "AWS_ACCESS_KEY_ID" => aws_access_key_id,
    "AWS_SECRET_ACCESS_KEY" => aws_secret_access_key,
    "AWS_SECURITY_TOKEN" => aws_security_token
  ...
  cwd "/tmp"
  user "root"
  command "aws rds revoke-db-security-group-ingress --db-security-group-name #{TARGET_DB_SG} --cidrip #{PUBLIC_IPADDRESS}/32 --region ap-northeast-1"
  only_if "cat #{RDS_SG_STATUS_FILE} | grep #{PUBLIC_IPADDRESS}"
end
